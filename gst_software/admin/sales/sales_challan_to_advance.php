...
<!--end firm-->	
  ...
  <!-- Content Wrapper. Contains page content -->
	...
	<?php
	$advance_no=$_GET['invoice_no'];
	$select_invoice_no = "select * from sales_invoice_info where challan_no='$advance_no' and company_code='$company_code'";
	$run_invoice = mysql_query($select_invoice_no);
	$fetchrow = mysql_fetch_array($run_invoice);
	$invoice_no1 = $fetchrow['invoice_no'];
	$advance_type=$_GET['invoice_type'];
	if($advance_type=='sales'){
	$table_name='sales_delivery_challan_info';
	}elseif($advance_type=='purchase'){
	$table_name='purchase_delivery_challan_info';
	}
	$que11="select * from $table_name where invoice_no='$advance_no' and invoice_status='Active' and company_code='$company_code'";
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
	...
	<div class="box-body">
				...
				<div class="col-md-12 ">
						...
						$advance_invoice_no=$row2['advance_invoice_no'];
						$val=substr($advance_invoice_no, 1);
						}
						?>
						  <label>Invoice No.</label>
						   <input type="text" name="invoice_no" placeholder="" value="<?php echo 'ADV-'.$val; ?>" class="form-control" readonly>
						</div>
				</div>
				<div class="col-md-12 ">				
					<div class="form-group col-md-5" >
					...
					<input type="radio" name="invoice_type" id="inv_type_1" value="purchase" <?php if($advance_type=='purchase'){ echo 'checked'; }else{ echo 'disabled'; } ?> >&nbsp;&nbsp;<b>Purchase</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="invoice_type" id="inv_type_2" value="sales" <?php if($advance_type=='sales'){ echo 'checked'; }else{ echo 'disabled'; } ?> >&nbsp;&nbsp;<b>Sales</b>
					</div>
				...
				<div class="col-lg-12">
					<div class="form-group col-lg-5">
					 ...
					 <select name="sales_person_name" id="sales_person_name" class="form-control select2">
					 <?php $i=1;
					 while($i<3){ ?>
					    <option value="<?php echo $i; ?>">Fisrt_<?php echo $i; ?></option>
						
						<?php $i=$i+1; }  ?>
					<option value="addmore"><a href="" data-toggle="modal" data-target="#myModal3">Add More</a></option>
					    </select>
    ...
    <div class="box-body table-responsive">
        <table id="example1" class="table table-bordered" style="background-color:white;">
			...
			<tbody>
			<?php
			$que12="select * from $table_name where invoice_no='$advance_no' and invoice_status='Active' and company_code='$company_code'";
...
$set=0;
for($j=0; $j<$count; $j++){
 $query1="update $table_name set invoice_no='$invoice_no',invoice_date='$invoice_date',invoice_reference='$invoice_reference',invoice_due_date='$invoice_due_date',invoice_firm_name='$invoice_firm_name',invoice_billing_address='$invoice_billing_address',invoice_shipping_address='$invoice_shipping_address',invoice_gstin_no='$invoice_gstin_no',invoice_place_of_supply='$invoice_place_of_supply',invoice_product_name='$item_product_name[$j]',invoice_description='$item_description[$j]',invoice_hsn='$item_hsn[$j]',invoice_quantity='$item_quantity[$j]',invoice_available_quantity='$item_avail_quantity[$j]',invoice_item_unit='$item_unit[$j]',invoice_rate='$item_price[$j]',invoice_rate1='$item_price1[$j]',invoice_price_fix='$item_price_fix[$j]',invoice_discount='$item_discount[$j]',invoice_discount_type='$item_discount_type[$j]',invoice_taxable='$item_tax_type[$j]',invoice_tax_type='$item_taxable[$j]',invoice_tax='$item_tax_amount[$j]',invoice_cgst='$item_cgst[$j]',invoice_cgst1='$item_cgst1[$j]',invoice_sgst='$item_sgst[$j]',invoice_sgst1='$item_sgst1[$j]',invoice_igst='$item_igst[$j]',invoice_igst1='$item_igst1[$j]',invoice_total='$item_total_amount[$j]',invoice_extra_expences='$invoice_extra_expences',invoice_sub_total='$invoice_sub_total',invoice_total_discount='$total_invoice_discount',invoice_total_discount_type='$total_discount_type',invoice_grand_total='$invoice_grand_total',invoice_payment_mode='$invoice_payment_mode',invoice_total_paid='$invoice_total_paid',remark='$remark',invoice_due_amount='$invoice_due_amount',invoice_customer_notes='$invoice_customer_notes',invoice_terms_and_conditions='$invoice_terms_and_conditions',invoice_type='$invoice_type',invoice_status='Active',invoice_order_no='$invoice_order_no',account_type='$account_type',account_name='$account_name',cheque_dd='$cheque_dd',cheque_dd_no='$cheque_dd_no',cheque_dd_amount='$cheque_dd_amount',cheque_dd_issue_date='$cheque_dd_issue_date',cheque_dd_clearing_date='$cheque_dd_clearing_date',transaction_type='$transaction_type',stock_quantity_rate_update='$stock_quantity_rate_update',challan_no='$advance_no',advance_no='$invoice_no',shipping_date='',order_status='Cancel',sales_person_name='$sales_person_name',sales_excutive_name='$sales_excutive_name',transport_name='$transport_name',invoice_status2='Advance' where invoice_no='$invoice_no1' and company_code='$company_code'";
...
$qry6 = "update sales_delivery_challan_info set invoice_quantity='$item_quantity[$j]',invoice_available_quantity='$stock_item_quantity',invoice_status2='Return' where invoice_no='$advance_no' and company_code='$company_code'";
mysql_query($qry6);
mysql_query($que5);
} }
...
$que2="update invoice_no set folder_id='$folder_id',advance_invoice_no='$advance_invoice_no' where company_code='$company_code'";
mysql_query($que2);
if($set>0){
