...
?>
	...
	<?php
		...
		if($inv_type == 'sales'){ $table = "sales_invoice_info"; }
		else{ $table = "purchase_invoice_info"; }
	    $select = "select * from $table where invoice_no='$invoice_no' and invoice_status='Active' and invoice_due_amount>0  and company_code='$company_code'";
		$run = mysql_query($select);
		$i=0;
		while($fetchrow = mysql_fetch_array($run)){
		   $invoice_firm_name = $fetchrow['invoice_firm_name'];
		   $invoice_due_amount[$i] = $fetchrow['invoice_due_amount'];
		   $payment_count = $fetchrow['payment_count'];
		   $i = $i+1;
		 }
        ...
        function customer_vendor(value){  	
            $.ajax({
              ...
              success: function(detail){
				   ...
				   $("#balance_amount").val('');
				   $("#transaction_type").html("<option value='Credit'>Credit</option>");
              }
            });
		...
		function myFunction(value)
			{
			  $.ajax({
			   ...
			   success:function(detail)
			   {
			    if(detail==1)
				{
			     get_content('sales/advance_pay_add');
				}
			   }
			   })
			}
    ...
    <div class="box box-primary my_border_top">
        ...
        <!--------------------------------###########################------------------------------------------->	        			
			    ...
			    <div class="col-sm-12 form-horizontal">	
				    ...
				    <div class="col-sm-6">
                    <a href="javascript:get_content('sales/cash_details')"><i class="fa fa-times" style="margin-left:100%;color:black"></i></a>
                    </div>
                    </div>
                    ...
                    <div class="col-sm-7">
						...
						<option value="">Select</option>
						<?php
						$que="select * from contact_master where contact_status='Active' and s_no='$invoice_firm_name' and company_code='$company_code'";
                    ...
                    <div class="col-sm-7">
                       <input type="text" name="total_amount" id="total_amount" value="" onchange="valid_amount(this.value)" Placeholder="Amount" class="form-control" required>
                    </div>
                    </div>
                    </div>
					...
					<div class="col-sm-3">	
			        <a href="javascript:post_content('sales/cash_details','id=<?php echo $s_no; ?>')"><button type="button" class="btn btn-primary">Cancel</button></a>
		            </div>
		            </div>	
			    </div>
	            </tbody>				
	       ...
	       <!-----------------------------------Expence Details Start----------------------------------->		   
				...
				<tbody>				
				<?php				
				$que="select * from sales_invoice_info where invoice_no='$invoice_no' and invoice_due_amount !='0' and invoice_status='Active' and company_code='$company_code'";
				$run=mysql_query($que);
				while($row=mysql_fetch_array($run)){
				...
				$select_pay_mode = "select * from bank_or_credit_card_info where s_no='$invoice_payment_mode' and company_code='$company_code'";
				$run = mysql_query($select_pay_mode);
				$fetchrow = mysql_fetch_array($run);
				$bank_account_name = $fetchrow['bank_account_name'];
				if($bank_account_name)
				{
				 $invoice_payment_mode = $bank_account_name;
				}
				...
				<tr>
				  ...
				  <th><?php echo $invoice_grand_total; ?></th>
				   ...
				   <th><a style="color:Red;" aria-hidden="true" onclick="if(window.confirm(' Do You Want Deleted..'))return myFunction('<?php echo $invoice_no; ?>')" class="fa fa-trash-o" href='#'> Delete</a></th>
				  <?php } } ?>
				</tr>					
				</tbody>
                </table>
