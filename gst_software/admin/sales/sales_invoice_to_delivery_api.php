...
$page_name='purchase_delivery_challan_draft_list.php';
...
invoice_place_of_supply,invoice_product_name,invoice_description,invoice_hsn,
invoice_quantity,invoice_available_quantity,invoice_item_unit,invoice_rate,
...
invoice_type,invoice_status,invoice_order_no,account_type,account_name,cheque_dd,cheque_dd_no,
cheque_dd_amount,cheque_dd_issue_date,cheque_dd_clearing_date,transaction_type,challan_type,
order_no,invoice_status2,order_return_reason,order_return_term,company_name,company_code,item_mrp) values('$invoice_no','$invoice_date','$invoice_reference',
...
'$item_tax_type[$i]','$item_tax_amount[$i]','$item_cgst[$i]','$item_cgst1[$i]','$item_sgst[$i]',
'$item_sgst1[$i]','$item_igst[$i]','$item_igst1[$i]','$item_total_amount[$i]','$invoice_extra_expences',
'$invoice_sub_total','$total_invoice_discount','$total_discount_type','$invoice_grand_total',
'$invoice_payment_mode','$invoice_total_paid','$remark','$invoice_due_amount','$invoice_customer_notes',
'$invoice_terms_and_conditions','$invoice_type','Active','No','$account_type','$account_name','$cheque_dd',
'$cheque_dd_no','$cheque_dd_amount','$cheque_dd_issue_date','$cheque_dd_clearing_date','$transaction_type',
'$challan_type','','No Invoice','','','$company_name','$company_code','$item_mrp[$i]')";
...
$que2="update invoice_no set delivery_challan_no='$delivery_challan_no' where company_code='$company_code'";
mysql_query($que2);
if($number==1){ $update_table ="sales_invoice_draft_info"; $var ="Delivery Challan(Invoice)"; } else{ $update_table = "sales_invoice_info"; $var ="Delivery Challan(Draft)"; }
$update = "update $update_table set challan_no='$invoice_no' where invoice_no='$invoice_no1' and company_code='$company_code'";
mysql_query($update);
if($save>0){
echo "<script>window.open('$page_name','_self');</script>";
} } }
