<?php  include("../../attachment/session.php"); ?>
  ...
  document.getElementById('taxable_'+sno).value=(parseFloat(quantity)*parseFloat(price)-parseFloat(disc_amt)).toFixed(0);
  ...
  document.getElementById('tax_amount_'+sno).value=(for_tax/100).toFixed(0);
  document.getElementById('item_total_'+sno).value=(parseFloat(parseFloat(quantity)*parseFloat(price))-parseFloat(disc_amt)+parseFloat(parseFloat(for_tax)/100)).toFixed(0);
  ...
  document.getElementById('tax_amount_'+sno).value=(for_tax0/100).toFixed(0);
  document.getElementById('item_total_'+sno).value=(parseFloat(parseFloat(quantity)*parseFloat(price))-parseFloat(disc_amt)+parseFloat(parseFloat(for_tax0)/100)).toFixed(0);
  }
  ...
  document.getElementById('taxable_'+sno).value=(parseFloat(quantity)*parseFloat(price)).toFixed(0);
  if(tax_type=='CGST&SGST'){
  var for_tax3=parseFloat(quantity)*parseFloat(price)*parseFloat(parseFloat(cgst)+parseFloat(sgst));
  document.getElementById('tax_amount_'+sno).value=(for_tax3/100).toFixed(0);
  document.getElementById('item_total_'+sno).value=(parseFloat(parseFloat(quantity)*parseFloat(price))+parseFloat(parseFloat(for_tax3)/100)).toFixed(0);
  }else if(tax_type=='IGST'){
  var for_tax4=parseFloat(quantity)*parseFloat(price)*parseFloat(igst);
  document.getElementById('tax_amount_'+sno).value=(for_tax4/100).toFixed(0);
  document.getElementById('item_total_'+sno).value=(parseFloat(parseFloat(quantity)*parseFloat(price))+parseFloat(parseFloat(for_tax4)/100)).toFixed(0);
  }
  }
  $('#click_total').click();
  }else{
  ...
  <script type="text/javascript">
     $( document ).ready(function() {
	  ...
	  payment_term2(pay_term_value);
	   ...
	   $("#estimate_edit_form").submit(function(e){
        e.preventDefault();
    var formdata = new FormData(this);
        $.ajax({
            url: software_link+"sales/new_estimate_edit_api.php",			
            type: "POST",
            data: formdata,
            ...
            success: function(detail){
			   ...
			   if(res[1]=='success'){
				   ...
				   post_content('sales/sales_estimate_list',res[2]);
            }
			}
         });
      });
	...
	$que11="select * from $table_name where invoice_no='$estimate_no' and invoice_status='Active' and company_code='$company_code'";
	$run11=mysql_query($que11) or die(mysql_error());
	$num = mysql_num_rows($run11);
	$number = 0;
	if($num<1)
	{
		 $que11 ="select * from $table_name2 where invoice_no='$estimate_no' and invoice_status='Active' and company_code='$company_code'";
		 $run11 = mysql_query($que11) or die(mysql_error());
		 $number=1;
	}
	...
	?>
    <!-- Main content -->
	<form method="post" onsubmit="return validate();" id="estimate_edit_form" enctype="multipart/form-data">
	 <input type="hidden" name="invoice_no" placeholder="" value="<?php echo $invoice_no; ?>" class="form-control" readonly>
	  <input type="hidden" name="remark" placeholder="" value="" class="form-control" readonly>
	  <input type="hidden" name="transaction_type" placeholder="" value="<?php echo $transaction_type; ?>" class="form-control" readonly>
	  <input type="hidden" name="invoice_order_no" placeholder="" value="<?php echo $invoice_order_no; ?>" class="form-control" readonly>
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border ">
              <h1 class="box-title" style="color:red">Estimate</h1>
			   <h1 class="box-title" style="float:right;">Estimate No: <?php echo $invoice_no; ?></h1>
            </div>
            <!-- /.box-header -->
<!------------------------------------------------Start Registration form--------------------------------------------------->
	<div class="box-body">
				<div class="col-md-12">
					<input type="radio" name="invoice_type" id="inv_type_1" value="purchase" <?php if($estimate_type=='purchase'){ echo 'checked'; }else{ echo 'disabled'; } ?> style="display:none;" ><input type="radio" name="invoice_type" id="inv_type_2" value="sales" <?php if($estimate_type=='sales'){ echo 'checked'; }else{ echo 'disabled'; } ?> style="display:none;" >
					<div class="form-group col-md-4" >
					  ...
					  <option value="">Select Firm</option>
							...
							?>
							<option <?php if($s_no==$invoice_firm_name){ echo "selected"; } ?> value="<?php echo $s_no; ?>"><?php echo $contact_company_name; ?></option>
							<?php
							}
							?>
					  </select>
				...
				<div class="col-md-12 ">	
					...
					<div class="form-group col-md-4" >
						...
						<textarea name="invoice_billing_address" id="invoice_billing_address" rows="3" class="form-control" style="resize:none;"><?php echo $invoice_billing_address; ?></textarea>
					</div>
					<div class="form-group col-md-4" >
						<label >Shipping Address</label>
						<textarea name="invoice_shipping_address" id="invoice_shipping_address" rows="3" class="form-control" style="resize:none;"><?php echo $invoice_shipping_address; ?></textarea>
					</div>
				</div>
				<div class="col-md-12 ">		
						<div class="form-group col-md-4" >
	...
	<?php
			$que12="select * from $table_name where invoice_no='$estimate_no' and invoice_status='Active' and company_code='$company_code'";
	...
	if($num12<1)
	{
		 ...
		 $run12 = mysql_query($que12); }
			...
			$sql2 = "select item_mrp_price from item_master join sales_estimate_info on item_master.s_no=sales_estimate_info.invoice_product_name where sales_estimate_info.s_no='$invoice_s_no'";
        ...
        <table class="table table-bordered" style="background-color:white;" id="table_1">
			...
			<tbody>
			 <tr id="<?php echo 'row_'.$serial; ?>">
					...
					<div>
							...
							<textarea name="item_description[]" id="<?php echo 'desc_'.$serial; ?>" class="form-control" style="border-color: Transparent;padding:0px;margin:0px; resize:none;" rows="1" ><?php echo $invoice_description; ?></textarea>
						</div>
					</div>
					</td>
					<td>
			...
			<tbody><tr>
				...
				<td>
					...
					<div class="col-md-12">
					 <div class="row">
					 <label id="for_cgst_<?php echo $serial; ?>" style="width:40px;font-size:12px;color:#423C9B;">CGST : </label>
		...
		<div class="form-group">
			...
			<button type="button" class="btn btn-success" onclick="Payment();" id='payment'>Payment</button>
			  	...
			  	<div class="col-md-12" style="display:none" id="Payment_mode">
