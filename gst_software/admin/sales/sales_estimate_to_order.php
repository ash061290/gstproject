<?php  include("../../attachment/session.php"); ?>
  ...
  function for_grandtotal(){
	 ...
	 if(extra_expence>0){
	 document.getElementById('invoice_sub_total').value=(parseFloat(add)-parseFloat(extra_expence)).toFixed(0);
	 }else{
	 document.getElementById('invoice_sub_total').value=add.toFixed(0);
	 }
	 ...
	 document.getElementById('invoice_grand_total').value=(parseFloat(add)-parseFloat(disc_amt11)).toFixed(0);
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(disc_amt11)).toFixed(0);
	 
	 if(paid_amount>0){
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(disc_amt11)-parseFloat(paid_amount)).toFixed(0);
	 }else{
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(disc_amt11)).toFixed(0);
	 }
	 }else if(invoice_discount=='Rs'){
	 document.getElementById('invoice_grand_total').value=(parseFloat(add)-parseFloat(discount_amount)).toFixed(0);
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(discount_amount)).toFixed(0);
	 if(paid_amount>0){
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(paid_amount)-parseFloat(discount_amount)).toFixed(0);
	 }else{
	 document.getElementById('invoice_due_amount').value=(parseFloat(add)-parseFloat(discount_amount)).toFixed(0);
	 }
	 }
	 }else{
	 document.getElementById('invoice_grand_total').value=add.toFixed(0);
	 if(paid_amount>0){
...
<!--end firm-->
	...
	$("#sales_estimate_to_order_form").submit(function(e){
        e.preventDefault();
    var formdata = new FormData(this);
        $.ajax({
            url: software_link+"sales/sales_estimate_to_order_api.php",			
            type: "POST",
            data: formdata,
...
<?php
	...
	while($row11=mysql_fetch_array($run11)){
	 ...
	 $select_term = "select contact_payment_terms from contact_master where s_no = '$invoice_firm_name' and company_code='$company_code'";
                  $run = mysql_query($select_term);
                   if($fetchr = mysql_fetch_array($run))
                     {  $payment_term = $fetchr['contact_payment_terms']; }
	?>
    <!-- Main content -->
	<form method="post" onsubmit="return validate();" id="sales_estimate_to_order_form" enctype="multipart/form-data">
	<input type="hidden" name="invoice_no" value="<?php echo 'SO-'.$val; ?>" />
	<input type="hidden" name="sales_order_no" value="<?php echo $sales_order_no; ?>"  />
	<input type="hidden" name="sales_estimate_no" value="<?php echo $estimate_no; ?>"  />
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border">
              <h1 class="box-title" style="color:red">Invoice</h1>
			   <h1 class="box-title" style="float:right;">Order No: <?php echo 'SO-'.$val; ?></h1>
            </div>
	<div class="box-body">
				<div class="col-md-12 ">				
					<div class="form-group col-md-4" >
					  <label>Firm Name <span style="color:red;">*</span></label>
					  <select name="invoice_firm_name" class="form-control select2" id="invoice_firm_name" onchange="firm_info(this.value,'<?php 
					  echo $company_code; ?>');" style="width:100%" required>
					        <?php
				...
				<div class="col-md-12">	
					<div class="form-group col-md-4">
					  ...
					  <input type="date" name="invoice_due_date" id="due_date" placeholder="Date" value="<?php echo $invoice_due_date; ?>" class="form-control" required>
					</div>
				...
				<div class="form-group col-md-4">
					...
					<div class="form-group col-lg-4">
					 <label >Sales Executive</label>
					  <input type="text" name="sales_excutive_name" placeholder="Sales Person Name" id="sales_excutive_name" value="" class="form-control" required>
					</div> </div>
