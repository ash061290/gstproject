    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="javascript:get_content('index')"><i class="fa fa-home"></i>Home</a></li>
		 <!--<li><a href="due_payment.php"><i class="fa fa-list"></i>All Due Transaction</a></li>-->
        <li><a href="javascript:get_payment('due_payment/receivable_payment')"><i class="fa fa-list"></i>Receivabale Payment</a></li>
		 <li><a href="javascript:get_payment('due_payment/defaulter')"><i class="fa fa-list"></i>Default Payment</a></li>
...
<script>
   ...
   else
   {
		 $.ajax({
             type:"POST",		   
		     url:software_link+"due_payment/ajax_payterm_search.php?pay_term_debit="+pay_term+"",
			 cache:false,
			 data:"pay_term_debit="+pay_term,
...
<script type="text/javascript">
	  ...
	  var to_date= document.getElementById('to_date').value;
	   $.ajax({
			  type: "POST",
              url: software_link+"due_payment/ajax_due_payment_date.php?debit_from_date="+from_date+"&debit_to_date="+to_date+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
   ...
   var to_date=current_year+'-12-31';
      $.ajax({
			  type: "POST",
              url: software_link+"due_payment/ajax_due_payment_all_month.php?debit_current_year="+current_year+"&from_date="+from_date+"&to_date="+to_date+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
