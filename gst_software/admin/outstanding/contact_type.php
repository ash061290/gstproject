...
<script>
   ...
   else
   {
		 $.ajax({
             type:"POST",		   
		     url: software_link+"due_payment/ajax_search.php?pay_term="+pay_term+"&contact_type="+contact_type+"",
			 cache:false,
...
<script type="text/javascript">
	  ...
	  var to_date= document.getElementById('to_date').value;
	   ...
	   $.ajax({
			  type: "POST",
              url: software_link+"due_payment/ajax_search.php?from_date="+from_date+"&to_date="+to_date+"&contact_type="+contact_type+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
	    ...
	    if(current_month=="all")
   {
    ...
    var to_date=current_year+'-12-31';
      $.ajax({
			  type: "POST",
              url: software_link+"due_payment/ajax_search.php?current_year="+current_year+"&contact_type="+contact_type+"&from_date="+from_date+"&to_date="+to_date+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
