<?php include("../../attachment/session.php"); 
	...
	<script type="text/javascript">
	  ...
	  if ((Date.parse(from_date) >= Date.parse(to_date))) {
	   ...
	   $.ajax({
			  type: "POST",
              url: software_link+"report/Purchase_date_ajax.php?from_date="+from_date+"&to_date="+to_date+"",
              cache: false,
	  ...
	  if(current_month =='all')
   {
    ...
    var to_date=current_year+'-12-31';
      ...
      $.ajax({
			  type: "POST",
              url: software_link+"report/Purchase_year_ajax.php?current_year="+current_year+"&from_date="+from_date+"&to_date="+to_date+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
   ...
   else
   {
      ...
      var to_date=current_year+'-'+current_month+'-'+last_day;
	   ...
	   document.getElementById('to_date').value = to_date;
             ...
             $.ajax({
			  type: "POST",
              url: software_link+"report/Purchase_year_ajax.php?from_date="+from_date+"&to_date="+to_date+"",
              cache: false,
              success: function(detail){
              $('#my_table1').html(detail);
              }
     ...
     function for_print()
 {
	 ...
	 $.ajax({
		     type: "POST",
			 url: software_link+"report/ajax_purchase_report_pdf.php",
			 data:"year="+year+"&month="+month+"&to_date="+to_date+"&from_date="+from_date+"",
			 cache:false,
			 success: function(detail){
				 alert(detail);
...
<div class="col-md-4">
   <label>Action</label><br/>
    ...
    <input type="button" name="excel" value="Print Excel" onclick="exportTableToExcel('printTable', 'Purchase Report')" class="btn btn-success" style=" float:right">
</div>
</div>
