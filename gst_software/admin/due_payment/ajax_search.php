...
<?php
 ...
 $query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id`  JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `account_info`.`invoice_due_amount` !='' AND `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `contact_master`.`contact_status`='Active' AND `contact_master`.`contact_contact_type`='$contact_type' AND `sales_invoice_info`.`invoice_date` >= '$from_date' AND `sales_invoice_info`.`invoice_date` <= '$to_date' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
 }  else
if(isset($_GET['current_year'])) 
{
 ...
 $to_date = $_GET['to_date'];
 $query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id`  JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `account_info`.`invoice_due_amount` !='' AND `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `contact_master`.`contact_status`='Active' AND `sales_invoice_info`.`invoice_date` >= '$from_date' AND `sales_invoice_info`.`invoice_date` <= '$to_date' AND `contact_master`.`contact_contact_type`='$contact_type' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
}
if(isset($_GET['pay_term']))
{
 $pay_term = $_GET['pay_term'];
 $query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id` JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `account_info`.`invoice_due_amount` !='' AND  `contact_master`.`contact_status`='Active' AND `contact_master`.`contact_contact_type`='$contact_type' AND `contact_master`.`contact_payment_terms`='$pay_term' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
}
$serial_no=0;
...
<th><!--<a aria-hidden='true' class='fa fa-edit' href='#'><span style='color:#2eb82e;font-size:15px;'> Edit </span></a> &nbsp;&nbsp;&nbsp;&nbsp;-->
<a aria-hidden='true' class='fa fa-trash' style="color:red;" onclick="return confirm('Do U Want Delete')" href='advance_invoice_delete.php?invoice_id=<?php echo $invoice_no; ?>'><span style='color:red; font-size:15px;'> Delete </span></a></th>
</tr> <?php } ?>
</tbody>
</table>
</div>
