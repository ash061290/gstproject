...
$to_date = $_GET['to_date'];
 $query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id`  JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `account_info`.`invoice_due_amount` !='' AND `contact_master`.`contact_status`='Active' AND `sales_invoice_info`.`invoice_date` >= '$from_date' AND `sales_invoice_info`.`invoice_date` <= '$to_date' AND `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `sales_invoice_info`.`invoice_date` DESC";
 }
$serial_no=0;
