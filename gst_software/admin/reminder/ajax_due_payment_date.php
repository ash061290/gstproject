  ...
  <table id="example4" class="table table-bordered table-striped">
   <thead class="my_background_color">
		 <tr>
		  ...
		  <?php //if (isset($_GET['credit_from_date']) && isset($_GET['credit_to_date'])) {	?>
		   <th>Action</th>
		   <?php //} ?>
		</tr>
	</thead>
	<tbody >
<?php
 if(isset($_GET['credit_from_date']) && ($_GET['credit_to_date']))
 {
 $from_date=$_GET['credit_from_date'];
 $to_date=$_GET['credit_to_date'];
 $query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id`  JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `sales_invoice_info`.`invoice_due_date`> CURDATE() AND `account_info`.`invoice_due_amount` !='' AND `contact_master`.`contact_status`='Active' AND `account_info`.`transaction_type`='Credit' AND `sales_invoice_info`.`invoice_date` >= '$from_date' AND `sales_invoice_info`.`invoice_date` <= '$to_date' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
 }  else
if(isset($_GET['debit_from_date']) && ($_GET['debit_to_date'])) 
{
$from_date=$_GET['debit_from_date'];
$to_date=$_GET['debit_to_date'];
$query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id`  JOIN `purchase_invoice_info` ON `purchase_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `account_info`.`invoice_due_amount` !='0.00' AND `account_info`.`invoice_total_paid` !='' AND `contact_master`.`contact_status`='Active' AND `purchase_invoice_info`.`invoice_date` >= '$from_date' AND `purchase_invoice_info`.`invoice_date` <= '$to_date' AND `account_info`.`date` <= '$to_date' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
}
if(isset($_GET['default_from_date']) && ($_GET['default_to_date']))
{
$from_date = $_GET['default_from_date'];
$to_date = $_GET['default_to_date'];
$query1="SELECT * FROM `contact_master` JOIN `account_info` ON `contact_master`.`s_no`=`account_info`.`customer_id` JOIN `sales_invoice_info` ON `sales_invoice_info`.`invoice_no`=`account_info`.`invoice_no` WHERE `sales_invoice_info`.`invoice_due_date`< CURDATE() AND `account_info`.`invoice_due_amount` !='0.00' AND `account_info`.`invoice_total_paid` !='' AND `contact_master`.`contact_status`='Active' AND `account_info`.`transaction_type`='Credit' AND `sales_invoice_info`.`invoice_date` >= '$from_date' AND `sales_invoice_info`.`invoice_date` <= '$to_date' and `contact_master`.`company_code`='$company_code' and `account_info`.`company_code`='$company_code' GROUP BY `account_info`.`invoice_no` ORDER BY `account_info`.`s_no` DESC";
}
$serial_no=0;
...
<?php //if(isset($_GET['credit_from_date']) && isset($_GET['credit_from_date'])) { ?>
