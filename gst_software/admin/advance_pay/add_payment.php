<?php include("../../attachment/session.php");?>
 ...
 <script>
  ...
  $.ajax({
			address: "POST",
			url: software_link+"advance_pay/ajax_get_item_description.php?value="+value+"&inv_type="+inv_type+"&company_code="+company_code+"",
			cache: false,
  ...
  if(value!=''){
	$.ajax({
		address: "POST",
		url: software_link+"advance_pay/get_payment_details.php?id="+value+"",
		cache: false,
		success: function(detail){
		var res = detail.split("|?|");
		var myvar=res[0];
  ...
  // this code is use for get firm information -----START-----
   function firm_info(value,company_code){
		$.ajax({
			address: "POST",
			url: software_link+"advance_pay/get_firm_details.php?id="+value+"&company_code="+company_code+"",
			cache: false,
			success: function(detail){
			var res = detail.split("|?|");
...
function place_ofsupply(value){
  $.ajax({
		address: "POST",
		url: software_link+"advance_pay/ajax_get_admin_place_of_supply.php",
		cache: false,
		success: function(detail){
		var res = detail.split("|?|");
...
for(i=1; i<sno; i++){
  if(value==res[1]){
	...
	function get_barcode(value,id){
       $.ajax({
			  type: "POST",
              url: software_link+"advance_pay/ajax_get_data_by_barcode.php?barcode="+value+"",
              cache: false,
              success: function(detail){
  ...
  <script src="../attachment/file_check.js"></script>
    <section class="content-header">
	 ...
	 <ol class="breadcrumb">
	   ...
	   <li><a href="javascript:get_content('sales/advance')"><i class="fa fa-search"></i>Advance Invoice</a></li>
	   <li><a href="javascript:get_content('sales/add_payment)"><i class="fa fa-plus"></i>New Invoice</a></li>
...
<!------------------------------------------------Start Registration form---------------------------------------------------->
	...
	<div class="col-md-12">
		...
		<?php		
		            if(isset($_GET['inv_type'])){
					    if($_GET['inv_type'] == 'sales'){
		            $query2="select sales_invoice_no from invoice_no where company_code='$company_code'";
					$res2=mysql_query($query2);
					while($row2=mysql_fetch_array($res2)){
					$sales_invoice_no=$row2['sales_invoice_no'];
					$val=substr($sales_invoice_no, 1);
					} } }
					 if($_GET['inv_type'] == 'purchase'){
				...
				<div class="col-md-12 ">	
					<div class="form-group col-md-5" >
					  ...
					  <input type="date" name="payment_date" id="invoice_date" value="<?php echo date('Y-m-d'); ?>" class="form-control" onchange="invoice_d_change(this.value);">
					</div>
				</div>
				...
				<div class="col-lg-6">
					<div class="form-group col-lg-5">
					 ...
					 <select name="payment_term" id="pay_term" class="form-control select2" style="width:100%" onchange="payment_term2(this.value)">
					   <option value="Due on receipt">Due on receipt</option>
						...
						<option value="Net-60Net-60"></option>
					...
					<div class="form-group col-md-5">
					  ...
					  <input type="date" name="delivery_date" id="due_date" value="" class="form-control">
					</div>
				</div>
				</div>
				<div class="col-md-12 ">		
				...
				<div class="col-lg-12">
				  ...
				  <input type="text" name="sales_person_name" placeholder="Sales Person Name" id="sales_person_name" value="" class="form-control">
					</div>
				</div>
				<div class="col-lg-12">
					<div class="form-group col-lg-5">
				...
				<div class="col-lg-12" style="display:none;">
					<div class="form-group col-lg-5">
					 <label> Payment Mode </label>
						...
						<option value="">Select</option>
						<?php
						$query4="select * from bank_or_credit_card_info where bank_status='Active' company_code='$company_code'";
...
$account_name = "Petty Cash";
 ...
 $bank_q = "select s_no from bank_or_credit_card_info where (bank_account_name='$account_name' OR credit_card_account_name='$account_name') and company_code='$company_code'";
...
$que3="insert into account_info(date,customer_id,payment_mode,bank_s_no,account_type,account_name,cheque_dd,cheque_dd_no,cheque_dd_amount,cheque_dd_issue_date,cheque_dd_clearing_date,transaction_type,invoice_no,invoice_grand_total,invoice_total_paid,invoice_due_amount,folder_name,upload_file,account_status,cheque_status,company_name,company_code) values('$payment_date','$invoice_firm_name','$payment_mode','$invoice_payment_mode','$account_type','$account_name','$cheque_dd','$cheque_dd_no','$cheque_dd_amount','$cheque_dd_issue_date','$cheque_dd_clearing_date','$transaction_type','$invoice_no','$invoice_grand_total','$invoice_total_paid','$invoice_due_amount','','','Active','$cheque_status','$company_name','$company_code')";
mysql_query($que3);
if($invoice_type == 'sales'){
$sales_invoice_no=$sales_invoice_no+1;
$que2="update invoice_no set sales_invoice_no='$sales_invoice_no'";
mysql_query($que2);
}
if($invoice_type == 'purchase'){
$purchase_invoice_no=$purchase_invoice_no+1;
$que2="update invoice_no set purchase_invoice_no='$purchase_invoice_no'";
mysql_query($que2);
}
if($save>0){
