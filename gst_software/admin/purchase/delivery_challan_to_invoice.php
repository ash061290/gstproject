...
<!--end firm-->	
  ...
  <div class="content-wrapper"> 
	...
	$que11="select * from $table_name where invoice_no='$challan_no' and invoice_status='Active'";
	$run11=mysql_query($que11) or die(mysql_error());
	$numrow = mysql_num_rows($run11);
	if($numrow<1){
	$que11="select * from $table_name2 where invoice_no='$challan_no' and invoice_status='Active'";
	$run11=mysql_query($que11) or die(mysql_error());
	}
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
	...
	<div class="box-body">
				...
				<div class="col-md-12 ">		
					...
					<div class="col-lg-12">
				  <?php $select_term = "select contact_payment_terms from contact_master where s_no = '$invoice_firm_name'";
    ...
    <div class="box-body table-responsive">
        <table id="example1" class="table table-bordered" style="background-color:white;">
			...
			<tbody>
			<?php
			$que12="select * from $table_name where invoice_no='$challan_no' and invoice_status='Active'";
			$run12=mysql_query($que12) or die(mysql_error());
			$numrow=mysql_num_rows($run12);
			$number=0;
			if($numrow<1){
	$que12="select * from $table_name2 where invoice_no='$challan_no' and invoice_status='Active'";
	$run12=mysql_query($que12) or die(mysql_error());
	$number=1;
	}
			$serial=0;
...
if($number==1){
$qry6 = "update purchase_delivery_challan_draft_info set invoice_quantity='$item_quantity[$j]',invoice_available_quantity='$stock_item_quantity' where invoice_no='$challan_no'";
}
if($number==0){
	$qry6 = "update purchase_delivery_challan_info set invoice_quantity='$item_quantity[$j]',invoice_available_quantity='$stock_item_quantity' where invoice_no='$challan_no'";
}
mysql_query($qry6);
mysql_query($que5);
}
...
$que2="update invoice_no set folder_id='$folder_id',purchase_invoice_no='$purchase_invoice_no'";
mysql_query($que2);

if($set>0){
  $update_sales_delivery = "update purchase_delivery_challan_info set invoice_status2='Invoiced' where invoice_no='$challan_no'";
   mysql_query($update_sales_delivery);
 $select_sales_order = "select order_no from purchase_delivery_challan_info where invoice_no='$challan_no'";
  $run = mysql_query($select_sales_order);
  $fetch = mysql_fetch_array($run);
  $order_no = $fetch['order_no'];
  if($invoice_due_amount == '0')
  {
    $update_sales_order_info ="update purchase_order_info set purchase_order_status='Paid Invoice' Where invoice_no='$order_no'"; 
  }
   if($invoice_due_amount < $invoice_grand_total)
  {
    $update_sales_order_info ="update purchase_order_info set purchase_order_status='Partially Paid' Where invoice_no='$order_no'"; 
  }
  if($invoice_grand_total == $invoice_due_amount)
  {
  $update_sales_order_info ="update purchase_order_info set purchase_order_status='Invoice Created' Where invoice_no='$order_no'";
  }
