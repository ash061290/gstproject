...
<?php
					...
					$purchase_invoice_no=$row2['purchase_invoice_no'];
		             //$sales_invoice_draft_no=$row2['sales_invoice_draft_no'];
					 $val=substr($purchase_invoice_no, 1);
					}
					?>
...
$("#new_invoice_add").submit(function(e){
    ...
    var formdata = new FormData(this);
        $.ajax({
            url: software_link+"purchase/new_invoice_api.php",			
            type: "POST",
            data: formdata,
            ...
            success: function(detail){
			   ...
			   if(res[1]=='success'){
				  ...
				  post_content('purchase/purchase_invoice_list',res[2]);
            }
			}
         });
      });
    ...
    var formdata = new FormData(this);
        $.ajax({
            url: software_link+"purchase/purchase_invoice_list_edit_api.php",			
            type: "POST",
            data: formdata,
            mimeTypes:"multipart/form-data",
            contentType: false,
            ...
            success: function(detail){
			   ...
			   if(res[1]=='success'){
	    post_content('purchase/purchase_invoice_list',res[2]);
		window.open("../gst_software/invoice_pdf/shree_lakshya_new.php?invoice_no="+res[2]+"&inv_type=purchase");
            }
			}
         });
      });
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="javascript:post_content('purchase/purchase_invoice_list','inv_type=sales')"></i>Purchase Invoice</a></li>
        <li class="active">Add Invoice</li>
      </ol>
    </section>
      <?php
        $s_no=$_GET['id'];
        $que="select * from purchase_invoice_new where s_no='$s_no' and company_code='$company_code'";
        ...
        while($row=mysql_fetch_array($run)){
			...
			$item_product_name = $row['invoice_product_name'];
			$item_description = $row['invoice_description'];
			$item_hsn = $row['invoice_hsn'];
			$item_quantity = $row['invoice_quantity'];
			$item_avail_quantity = $row['invoice_available_quantity'];
			$item_unit = $row['invoice_item_unit'];
			$item_price = $row['invoice_rate'];
			$item_price1 = $row['invoice_rate1'];
			$item_price_fix = $row['invoice_price_fix'];
			$item_discount = $row['invoice_discount'];
			$item_discount_type = $row['invoice_discount_type'];
			$item_taxable = $row['invoice_taxable'];
			$item_tax_type = $row['invoice_tax_type'];
			//$item_tax_amount = $row['invoice_tax_amount'];
			$item_cgst = $row['invoice_cgst'];
			$item_cgst1 = $row['invoice_cgst1'];
			$item_sgst = $row['invoice_sgst'];
			$item_sgst1 = $row['invoice_sgst1'];
			$item_igst = $row['invoice_igst'];
			$item_igst1 = $row['invoice_igst1'];
			//$item_total_amount = $row['invoice_total_amount'];
			$invoice_extra_expences = $row['invoice_extra_expences'];
			$invoice_sub_total = $row['invoice_sub_total'];
			//$total_invoice_discount = $row['invoice_total_invoice_discount'];
			$total_discount_type = $row['invoice_total_discount_type'];
	...
	<div class="box-body">
		...
		<input type="hidden" name="purchase_invoice_no"  value="<?php echo $purchase_invoice_no; ?>" />
              ...
              <div class="col-md-12">
					<div class="col-md-4">
					 ...
					 <select name="invoice_firm_name" class="form-control select2" id="invoice_firm_name" onchange="firm_info(this.value,'<?php echo $company_code; ?>');" style="width:100%" required>
					  <option value="<?php echo $invoice_firm_name; ?>"><?php echo $invoice_firm_name; ?></option>
					        <?php
							$qry="select * from contact_master where contact_status='Active' and contact_contact_type='Vendor' and company_code='$company_code'";
					...
					<div class="form-group col-md-4">
					  ...
					  <select class="select2" name="invoice_place_of_supply" id="invoice_place_of_supply" onchange="place_ofsupply(this.value);" style="width:100%" required>
					        <option value="<?php echo $invoice_place_of_supply;?>"><?php echo $invoice_place_of_supply;?></option>
				...
				<div class="col-md-12 ">				
					<div class="form-group col-md-4" >
						...
						<textarea name="invoice_billing_address" id="invoice_billing_address" value="<?php echo $invoice_billing_address; ?>" rows="1" class="form-control"></textarea>
					</div>			
					<div class="form-group col-md-4" >
						<label >Shipping Address</label>
						<textarea name="invoice_shipping_address" id="invoice_shipping_address" value="<?php echo $invoice_shipping_address; ?>" rows="1" class="form-control"></textarea>
					</div>
					<div class="form-group  col-md-3">
				    	 <label class="control-label" style="text-align:left;">Choose Image</label>
				
					  <input type="file" name="upload_file" id="upload_file" placeholder="" onchange="check_file_type(this,'upload_file','purchase_image','image');" class="form-control" accept=".gif, .jpg, .jpeg, .png" value="">
					  	</div>
				      <div class="col-md-1">	
				      <img id="purchase_image" src=<?php echo $image_path."Profile.png"; ?> width='60px' height='60px'>
					</div>
	...
	<input type="hidden" id="sample2" value="1" />
        <table class="table table-bordered">
            <thead class="my_background_color">
                <tr id="row_1">
					...
					<th style="width:70px;">MRP (<i class="fa fa-inr" aria-hidden="true">)</th>
			 ...
			 <tr>
					...
					<td>
					  <div class="form-group">
							...
							<?php
							$query="select model_no,s_no from item where item_status='Active' and company_code='$company_code'";
		...
		<div class="box-footer">
					...
					<div class="col-md-2">
					<div class="form-group">
						<input type="submit" name="save" value="Save" class="form-control btn btn-success" onclick="form_submit()">
					</div>
					</div>
					<div class="col-md-2">
					<div class="form-group" >
