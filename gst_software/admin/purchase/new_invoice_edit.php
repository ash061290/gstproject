...
for(i=1; i<sno; i++){
  ...
  <div class="content-wrapper"> 
	...
	$runrow = mysql_num_rows($run11);
	$number=0;
	if($runrow<1){
		$que11="select * from $table_name2 where invoice_no='$invoice_no' and invoice_status='Active' and company_code='$company_code'";
	    $run11=mysql_query($que11) or die(mysql_error());
		$number=1;
	}
...
<!------------------------------------------------Start Registration form--------------------------------------------------->
    ...
    <div class="box-body table-responsive">
       <div id="item_table" class="table table-bordered" style="background-color:white">
			<?php
			if($number==1){$table_name = $table_name2; } else{ $table_name=$table_name;}
			...
			$sql2 = "select item_mrp_price from item_master join purchase_invoice_info on item_master.s_no=purchase_invoice_info.invoice_product_name where purchase_invoice_info.s_no='$invoice_s_no'";
			$run22 = mysql_query($sql2);
			$numrow = mysql_num_rows($run22);
			if($numrow<1)
			{
				$sql2 = "select item_mrp_price from item_master join purchase_invoice_draft_info on item_master.s_no=purchase_invoice_draft_info.invoice_product_name where purchase_invoice_draft_info.s_no='$invoice_s_no'";
				$run22 = mysql_query($sql2);
			...
			<tbody><tr>
					...
					<div class="col-md-12">
					 <div class="row">
						<label id="<?php echo 'for_cgst_'.$serial; ?>" style="font-size:12px;color:#423C9B;">CGST : </label>
						...
						<label id="<?php echo 'for_sgst_'.$serial; ?>" style="font-size:12px;color:#423C9B;">SGST :</label>
						...
						<label id="<?php echo 'for_igst_'.$serial; ?>" style="font-size:12px;color:#423C9B;">IGST : </label>
...
$stock_quantity_rate_update='';
	...
	else{ $table_name='purchase_invoice_info'; }
