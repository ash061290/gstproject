...
<div class="wrapper">
  ...
  $.ajax({
			address: "POST",
			url: software_link+"purchase/ajax_get_item_description.php?value="+value+"&inv_type="+inv_type+"&company_code="+company_code+"",
			cache: false,
  ...
  // this code is use for get firm information -----START-----
   function firm_info(value,company_code){
		$.ajax({
			address: "POST",
			url: software_link+"purchase/get_firm_details.php?id="+value+"&company_code="+company_code+"",
			cache: false,
			success: function(detail){
			var res = detail.split("|?|");
  ...
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="purchase_order_list.php"><i class="fa fa-plus"></i> Add New Order</a></li>
        <li class="active"><i class="fa fa-plus"></i> Create Delivery Challan</a></li>
      </ol>
    </section>	<!---*****************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************-->
	<?php
    ...
    $table_name2 = 'purchase_order_draft_info';
	}
	$que11="select * from $table_name where invoice_no='$order_no' and invoice_status='Active' and company_code='$company_code'";
	$run11=mysql_query($que11) or die(mysql_error());
	$numrow = mysql_num_rows($run11);
	$number=0;
	if($numrow<1)
	{
	$que11="select * from $table_name2 where invoice_no='$order_no' and invoice_status='Active' and company_code='$company_code'";
	$run11=mysql_query($que11) or die(mysql_error());
	$number=1;
	}
	...
	while($row11=mysql_fetch_array($run11)){
			$s_no=$row11['s_no'];
			$invoice_order_no=$row11['invoice_no'];
      ...
      <div class="row">
          ...
          <div class="box box-primary my_border_top">
            <div class="box-header with-border ">
              <h1 class="box-title" style="color:red">Purchase Order To Invoice</h1>
            </div>
            <!-- /.box-header -->
<!------------------------------------------------Start Registration form--------------------------------------------------->
	...
	<div class="col-md-12">
		<div class="form-group">
					...
					<input type="radio" name="invoice_type" id="inv_type_1" value="purchase" <?php if($order_type=='purchase'){ echo 'checked'; }else{ echo 'disabled'; } ?> >&nbsp;&nbsp;<b>Purchase</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="invoice_type" id="inv_type_2" value="sales" <?php if($order_type=='sales'){ echo 'checked'; }else{ echo 'disabled'; } ?> >&nbsp;&nbsp;<b>Sales</b>
					</div>
					</div>
				<div class="col-md-12 ">
						...
						<?php
					    $query2="select purchase_invoice_no from invoice_no where company_code='$company_code'";
						$res2=mysql_query($query2);
						if($row2=mysql_fetch_array($res2)){
					    $invoice_no2=$row2['purchase_invoice_no'];
						$val=substr($invoice_no2, 1);
						}
						?>
	...
	<input type="date" name="invoice_due_date" id="due_date" value="<?php echo $invoice_due_date; ?>"  class="form-control">
					</div>
				</div>
				</div>
				<div class="col-md-12 ">		
				...
				<div class="col-lg-12">
					<div class="form-group col-lg-5">
					 <label >Sales Person</label>
				      <input type="sales_person_name" name="sales_person_name" class="form-control" id="sales_person_name" placeholder="Sales Person Name" />
							  </div>
					</div>
	...
	<?php
			$que12="select * from $table_name where invoice_no='$order_no' and invoice_status='Active' and company_code='$company_code'";
			$run12=mysql_query($que12) or die(mysql_error());
			$num12 = mysql_num_rows($run12);
	if($num12<1){
		 $que12 = "select * from $table_name2 where invoice_no='$order_no' and invoice_status='Active' and company_code='$company_code'";
		 $run12 = mysql_query($que12); }
			...
			$sql2 = "select item_mrp_price from item_master join $table_name on item_master.s_no=$table_name.invoice_product_name where $table_name.s_no='$invoice_s_no'";
			$run22 = mysql_query($sql2);
			$numrow22 = mysql_num_rows($run22);
			if($numrow22<1)
			{
				$sql2 = "select item_mrp_price from item_master join $table_name2 on item_master.s_no=$table_name2.invoice_product_name where $table_name2.s_no='$invoice_s_no'";
		...
		<br />
			...
			<div class="box-footer">
					...
					<div class="col-md-4">
					<div class="form-group">
						<input type="submit"  name="save" value="Save" class="form-control btn-default">
					</div>
					</div>
					<div class="col-md-4">
					<div class="form-group" >
						<input type="submit" name="save_and_print" value="Save and Print" class="form-control btn-default">
					</div>
					</div>
					<div class="col-md-2"></div>
					</div>
...
$invoice_mrp_price = $_POST['item_mrp'];
if(isset($_POST['save'])){
...
$set=0;
for($i=0; $i<$count; $i++){
 $query1="insert into $table_name(invoice_no,invoice_date,invoice_reference,invoice_due_date,invoice_firm_name,invoice_billing_address,invoice_shipping_address,invoice_gstin_no,invoice_place_of_supply,invoice_product_name,invoice_description,invoice_hsn,invoice_quantity,invoice_available_quantity,invoice_item_unit,invoice_rate,invoice_rate1,invoice_mrp_price,invoice_price_fix,invoice_discount,invoice_discount_type,invoice_taxable,invoice_tax_type,invoice_tax,invoice_cgst,invoice_cgst1,invoice_sgst,invoice_sgst1,invoice_igst,invoice_igst1,invoice_total,invoice_extra_expences,invoice_sub_total,invoice_total_discount,invoice_total_discount_type,invoice_grand_total,invoice_payment_mode,invoice_total_paid,remark,invoice_due_amount,invoice_customer_notes,invoice_terms_and_conditions,invoice_type,invoice_status,invoice_order_no,account_type,account_name,cheque_dd,cheque_dd_no,cheque_dd_amount,cheque_dd_issue_date,cheque_dd_clearing_date,transaction_type,stock_quantity_rate_update,payment_count,challan_no,shipping_date,order_status,invoice_status2,company_name,company_code) values('$invoice_no','$invoice_date','$invoice_reference','$invoice_due_date','$invoice_firm_name','$invoice_billing_address','$invoice_shipping_address','$invoice_gstin_no','$invoice_place_of_supply','$item_product_name[$i]','$item_description[$i]','$item_hsn[$i]','$item_quantity[$i]','$item_avail_quantity[$i]','$item_unit[$i]','$item_price[$i]','$item_price1[$i]','$invoice_mrp_price[$i]','$item_price_fix[$i]','$item_discount[$i]','$item_discount_type[$i]','$item_taxable[$i]','$item_tax_type[$i]','$item_tax_amount[$i]','$item_cgst[$i]','$item_cgst1[$i]','$item_sgst[$i]','$item_sgst1[$i]','$item_igst[$i]','$item_igst1[$i]','$item_total_amount[$i]','$invoice_extra_expences','$invoice_sub_total','$total_invoice_discount','$total_discount_type','$invoice_grand_total','$invoice_payment_mode','$invoice_total_paid','$remark','$invoice_due_amount','$invoice_customer_notes','$invoice_terms_and_conditions','$invoice_type','Active','No','$account_type','$account_name','$cheque_dd','$cheque_dd_no','$cheque_dd_amount','$cheque_dd_issue_date','$cheque_dd_clearing_date','$transaction_type','','1','$order_no','','Package','Order Approved','$company_name','$company_code')";

  $que11="select * from purchase_order_info where invoice_no='$order_no' and invoice_status='Active' and company_code='$company_code'";
	$run11=mysql_query($que11) or die(mysql_error());
	$numrow = mysql_num_rows($run11);
	if($numrow<1)
	{
	$que11="update purchase_order_draft_info set purchase_order_status='Invoice Created' where invoice_no='$order_no' and company_code='$company_code'";
	mysql_query($que11) or die(mysql_error());
	}
	else{
 $update_purchase_order = "update purchase_order_info set purchase_order_status='Invoice Created' where invoice_no='$order_no' and company_code='$company_code'";
 mysql_query($update_purchase_order);
...
$que3="insert into account_info(date,customer_id,payment_mode,bank_s_no,account_type,account_name,cheque_dd,cheque_dd_no,cheque_dd_amount,cheque_dd_issue_date,cheque_dd_clearing_date,transaction_type,invoice_no,invoice_grand_total,invoice_total_paid,invoice_due_amount,folder_name,upload_file,account_status,cheque_status,company_name,company_code) values('$invoice_date','$invoice_firm_name','','$invoice_payment_mode','$account_type','$account_name','$cheque_dd','$cheque_dd_no','$cheque_dd_amount','$cheque_dd_issue_date','$cheque_dd_clearing_date','$transaction_type','$invoice_no','$invoice_grand_total','$invoice_total_paid','$invoice_due_amount','','','Active','Clear','$company_name','$company_code')";
mysql_query($que3);
$purchase_invoice_no=$invoice_no2+1;
