...
<?php
if($search_by != 'all'){
  $que="select * from purchase_estimate_info where invoice_status='Active' AND estimate_status2='$search_by' and company_code='$company_code' GROUP BY invoice_no"; }
 if($search_by =='Draft'){
    $que="select * from purchase_estimate_draft_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no"; }
if($search_by == 'all') {
$que="select * from purchase_estimate_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no";
 $que2 ="select * from purchase_estimate_draft_info where invoice_status='Active' and company_code='$company_code' group by invoice_no"; }
...
<tr  align='center' >
	...
	<th><a href="purchase_estimate_list.php?purchase_id=<?php echo $estimate_no; ?><?php if($search_by =='Draft'){ echo "&estimate_type=draft";} ?>"><?php echo $estimate_no; ?></a></th>
	<th><?php echo $invoice_reference; ?></th>
	<th><?php echo $contact_tittle_name.'   '.$contact_first_name.'   '.$contact_last_name; ?></th>
	<th><span style="color:#6B823E;"><?php echo $estimate_status2." (".$estimate_status.")"; ?></span></th>
...
<tr  align='center' >
	...
	<th><?php echo $estimate_date; ?></th>
	<th><a href="purchase_estimate_list.php?purchase_id=<?php echo $estimate_no; ?>"><?php echo $estimate_no; ?></a></th>
	<th><?php echo $invoice_reference; ?></th>
	<th><?php echo $contact_tittle_name.'   '.$contact_first_name.'   '.$contact_last_name; ?></th>
	<th><span style="color:#6B823E;"><?php echo $estimate_status2." (".$estimate_status.")"; ?></span></th>
	<th><?php echo $estimate_expire; ?></th>
...
?>
<?php
//purchase_invoice_info status change
if(isset($_GET['purchase_invoice'])) 
{ 
  $status_value = $_GET['purchase_invoice'];
 ?>
  <table id="example1" class="table table-bordered table-striped">
                <thead class="my_background_color">
                <tr>
...
<?php
 ...
 {
 $que="select * from purchase_invoice_draft_info where invoice_status='Active' AND invoice_status2='Draft' and company_code='$company_code' GROUP BY invoice_no";
 }
 if($status_value == "all")
 {
   $que="select * from purchase_invoice_info where invoice_status='Active' and company_code='$company_code' GROUP BY purchase_invoice_info.invoice_no";
 ...
 {
   $que="select * from purchase_invoice_info where invoice_status='Active' AND invoice_status2='$status_value' and company_code='$company_code' GROUP BY invoice_no";
 }
 if($status_value=='Overdue')
 {
   $que="select * from purchase_invoice_info where invoice_status='Active' and company_code='$company_code' and invoice_due_date < CURDATE() GROUP BY invoice_no";
 }
 if($status_value =='Advance')
 {
   $que="select * from purchase_invoice_info where invoice_status='Active' AND advance_no!='' and company_code='$company_code' GROUP BY invoice_no"; 
 }
...
<?php } ?>
 ...
 //end sales invoice
   ...
   if(isset($_GET['purchase_delivery_challan_status']))
   {
   ?>
     <table id="example1" class="table table-bordered table-striped">
                <thead class="my_background_color">
				...
				<tbody>
   <?php
     $status_value = $_GET['purchase_delivery_challan_status'];
	 if($status_value == 'all'){
  $que="select * from purchase_delivery_challan_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no";
}
if($status_value=='Draft')
{
    $que="select * from purchase_delivery_challan_draft_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no";
}
if($status_value == 'Invoiced' || $status_value == 'Unpaid' || $status_value == 'No Invoice' || $status_value =='Advance' || $status_value == 'Return' || $status_value == 'Delivered')
 {
  $que="select * from purchase_delivery_challan_info where invoice_status='Active' AND invoice_status2='$status_value' and company_code='$company_code' GROUP BY invoice_no";
 }
 if($status_value=='Partially Paid')
 {
  $que = "select purchase_delivery_challan_info.s_no,purchase_delivery_challan_info.invoice_no,purchase_delivery_challan_info.challan_type,purchase_delivery_challan_info.invoice_date,purchase_delivery_challan_info.invoice_reference,purchase_delivery_challan_info.invoice_firm_name,purchase_delivery_challan_info.invoice_status,purchase_delivery_challan_info.invoice_grand_total,purchase_delivery_challan_info.invoice_status2,purchase_delivery_challan_info.invoice_type from purchase_delivery_challan_info join purchase_invoice_info on purchase_delivery_challan_info.invoice_no=purchase_invoice_info.challan_no where purchase_invoice_info.challan_no !='' and purchase_invoice_info.invoice_due_amount < purchase_invoice_info.invoice_grand_total and purchase_invoice_info.invoice_total_paid !='' and purchase_invoice_info.invoice_due_amount!='0' and purchase_delivery_challan_info.invoice_status2='Delivered' and purchase_delivery_challan_info.company_code='$company_code'";
 }
...
<?php 
   ...
   if(isset($_GET['purchase_order_info_status']))
   {
      $status_value = $_GET['purchase_order_info_status'];
	  ?>
	   <table id="example1" class="table table-bordered table-striped">
                <thead class="my_background_color">
                <tr>
...
if( $status_value=='Draft')
{
  $que = "select * from purchase_order_draft_info where invoice_status='Active' and company_code='$company_code' order by invoice_no";
}
if($status_value=='All')
{
$que="select * from purchase_order_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no";
 $que2 = "select * from purchase_order_draft_info where invoice_status='Active' and company_code='$company_code' order by invoice_no";
}
if($status_value =='Pending Invoice')
{
$que="select * from purchase_order_info where invoice_status='Active' AND purchase_order_status='Challan Created' and company_code='$company_code' GROUP BY invoice_no";
}
if($status_value=='Invoice Created' || $status_value=='Delivered' || $status_value=='Return' || $status_value=='Partially Paid')
{
$que="select * from purchase_order_info where invoice_status='Active' AND purchase_order_status='$status_value' and company_code='$company_code' GROUP BY invoice_no";
}
$run=mysql_query($que) or die(mysql_error());
$serial_no=0;
while($row=mysql_fetch_array($run)){
...
<tr  align='center' >
	<th><?php echo $invoice_date; ?></th>
	<th><a href="purchase_order_list.php?purchase_order_no=<?php echo $order_no; ?>"><?php echo $order_no; ?></a></th>
	<th><?php echo $ref; ?></th>
	<th><?php echo $contact_tittle_name.'   '.$contact_first_name.'   '.$contact_last_name; ?></th>
	<th><span style="color:#6B823E;"><?php echo $purchase_order_status; ?></span></th>
	<th><?php echo $invoice_due_date; ?></th>
	<th><?php echo $invoice_grand_total; ?></th>
</tr>
<?php } ?>

<?php if($status_value=='All'){
$run=mysql_query($que2) or die(mysql_error());
$serial_no=0;
while($row=mysql_fetch_array($run)){
		$s_no=$row['s_no'];
		$invoice_date1=$row['invoice_date'];
...
<?php
   if($payment_type=="all")
   {
    $qry ="SELECT * FROM purchase_invoice_info where invoice_status='Active' and company_code='$company_code' GROUP BY invoice_no"; 
   }
   if($payment_type=="Invoiced")
   {
		$qry ="SELECT * FROM purchase_invoice_info where challan_no='' AND advance_no= '' and company_code='$company_code' GROUP BY invoice_no";
    }
	if($payment_type == "Partially")
	{
	$qry = "select * from purchase_invoice_info where invoice_due_amount!='0' and company_code='$company_code' and invoice_due_amount!=invoice_grand_total";
	}
	if($payment_type == "Advance")
	{
	$qry = "select * from purchase_invoice_info where advance_no!='' and company_code='$company_code' and invoice_due_amount!='0'";
	}
	if($payment_type == "Invoiced")
	{
	$qry = "select * from purchase_invoice_info where challan_no ='' and company_code='$company_code' and advance_no='' and invoice_due_amount!='0'";
	}
	if($payment_type == "Challan Invoice")
	{
	$qry = "select * from purchase_invoice_info where challan_no !='' and company_code='$company_code' and invoice_due_amount!='0'";
	}
