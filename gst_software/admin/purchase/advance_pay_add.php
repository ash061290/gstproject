  ...
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="#"><i class="fa fa-list"></i>Cash Details</a></li>
...
<script>
        function customer_vendor(value){  	
            $.ajax({
			  type: "POST",
              url: software_link+"purchase/ajax_customer_wise_invoice_details.php?customer_id="+value+"",
              cache: false,
        ...
        <!--------------------------------###########################------------------------------------------->	        			
			    ...
			    <div class="col-sm-12 form-horizontal">	
                    ...
                    <div class="col-sm-7">
						...
						<option value="">Select</option>
						<?php
						  $select_advance_sales = "select * from purchase_invoice_info where invoice_status='Active' and advance_no!='' and invoice_due_amount>0 and company_code='$company_code'";
						  $run = mysql_query($select_advance_sales);
						  while($fetchrow = mysql_fetch_array($run))
						  {
						  $contact_firm_name=$fetchrow['invoice_firm_name'];
						$que="select * from contact_master where contact_status='Active' and s_no='$contact_firm_name' and company_code='$company_code'";
	       ...
	       <!-----------------------------------Expence Details Start----------------------------------->		   
				...
				<tbody>				
				<?php				
				$que="select * from purchase_invoice_info where advance_no!='' and invoice_due_amount !='0' and invoice_status='Active' and company_code='$company_code'";
				$run=mysql_query($que);
				while($row=mysql_fetch_array($run)){
...
<?php
	...
	$fetchrow = mysql_fetch_array($run);
	 $s_no = $fetchrow['s_no'];
	$update_sales_table = "update sales_invoice_info set invoice_payment_mode='$bank_s_no' where s_no='$s_no' and company_code='$company_code'";
    ...
    echo "<script>window.open('advance_pay_add.php','_self');</script>";
}
}
?>
<script>
